...
library(tidyverse)
ggplot(data = OnlineNewsPopularity) +
  geom_histogram(mapping = aes(x=shares, y = stat(density)), bins = 50) +
  geom_density(mapping = aes(x = shares)) +
  scale_x_log10()

ggplot(data = OnlineNewsPopularity) +
  geom_bar(mapping = aes(x = shares), stat = "bin") +
  scale_x_log10()
...
ggplot(data = OnlineNewsPopularity) +
  geom_point(mapping= aes(x = kw_avg_avg, y = shares)) +
  geom_smooth(mapping = aes(x = kw_avg_avg, y = shares), method = "lm",
